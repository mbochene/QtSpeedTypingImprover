    ...
    , ui(new Ui::Window)
{
    ...
    connect(ui->endBackButton, &QPushButton::released, this, [=](){ ui->stackedWidget->setCurrentIndex(0); });
}
...
void Window::handleGameReady(const QString& gameMode, const GameState& state, const QString& newWord)
{
    ...
    const bool speedButtonsVisible = gameMode.compare(challengeText) != 0;
...
void Window::handleGameOver(const GameOverScore& score)
{
   ui->endNetWPMLabel->setText(QString("Net WPM: ") + QString::number(score.netWPM));
   ui->endErrorsLabel->setText(QString("Errors: ") + QString::number(score.typingErrors));
   ui->endTimeLabel->setText(timeText + QString::number(score.timeSec));
   ui->endTypedCharactersLabel->setText(QString("Typed characters: ") + QString::number(score.typedEntries));
...
void Window::updateWidgets(const GameState& state)
{
    ...
    if(this->gameMode.compare(challengeText) != 0)
